...
public class SlavePackage extends Applet{
	...
	private void credit(APDU apdu){
		...
		requestShare(buffer, CREDIT, amount);
	...
	private void debit(APDU apdu){
		...
		requestShare(buffer, DEBIT, amount);
		apdu.setOutgoingAndSend((short)0, outLen);
	}
	
	private void requestShare(byte[] buffer, byte method, short amount){
		...
		MasterInterface sio = (MasterInterface)(JCSystem.getAppletShareableInterfaceObject(aid, (byte)01));
